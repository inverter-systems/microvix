package com.topinternacional.linx.services.imp.copy;
...
import com.topinternacional.linx.dto.Produto;
import com.topinternacional.linx.dto.Registro;
import com.topinternacional.linx.dto.Vendedor;
...
import com.topinternacional.linx.service.NlService;
...
public class NotaFiscalService extends BasicService {
	...
	public Long getCodPessoa(Long idExec, NotaFiscal nf) throws ClienteException {
		...
		if (isJuridica) {
			...
			if (codPessoa ==  null) {
				log.save(new Log(idExec, "Pessoa não encontrada, sistema tentará cria o cadastro da pessoa física ("+cnpjCpf+") com as informações do Microvix"));
				codPessoa = nlService.criarPessoaJuridica(idExec, cli);
			}
		} else {
			codPessoa = psFisicasRep.findByNumCpf(cnpjCpf);
			if (codPessoa ==  null) {
				log.save(new Log(idExec, "Pessoa não encontrada, sistema tentará cria o cadastro da pessoa juridica ("+cnpjCpf+") com as informações do Microvix"));
				try {
					codPessoa = nlService.criarPessoaFisica(idExec, cli);
				} catch (DataIntegrityViolationException e) {
					log.save(new Log(idExec, "Erro ao tentar criar a pessoa no N&L - "+e.getMessage()));
					throw new ClienteException();
				}
			}
		}
		
		log.save(new Log(idExec, "Retornando o código da pessoa ("+codPessoa+") no sistema N&L"));
		return codPessoa;
	}
	...
	public NotaFiscal getNotaFiscal(String id, Integer codUnidade) throws IOException {
		...
		if (!itens.isEmpty()) {
			...
			vendedor = Vendedor.getVendedor(soap.consultar(Metodo.LinxVendedores, getRegistrosVendedor(uc.getCnpj(), itens.get(0).getCodVendedor())));
		}
	...
	private ArrayList<Registro> getRegistrosCliente(String cnpj, Integer codClienteMicrovix) {
		...
		registro.getColunas().add(new Coluna("cod_cliente", codClienteMicrovix == null ? " " : codClienteMicrovix.toString()));
