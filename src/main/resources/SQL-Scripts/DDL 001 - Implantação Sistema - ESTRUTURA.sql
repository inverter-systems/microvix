...
SELECT CAST(IE.COD_ITEM AS VARCHAR2(50)) CODIGO
     ...
     , COALESCE(SUBSTR(IE.COD_REF_FORNECEDOR, 1, 20), 'SEM CADASTRO') REFERENCIA
     , COALESCE(SUBSTR(IE.COD_REF_FORNECEDOR, 1, 40), 'SEM CADASTRO') COD_AUXILIAR
...
COMMENT ON COLUMN DBAINV.TOPT_LINX_EXEC.ID IS 'ID da execução';
COMMENT ON COLUMN DBAINV.TOPT_LINX_EXEC.NOME IS 'Nome da execução';
COMMENT ON COLUMN DBAINV.TOPT_LINX_EXEC.DTA_INICIO IS 'Data de inicio da execução';
COMMENT ON COLUMN DBAINV.TOPT_LINX_EXEC.HORA IS 'Hora da execução';
COMMENT ON COLUMN DBAINV.TOPT_LINX_EXEC.MINUTO IS 'Minuto da execução';
COMMENT ON COLUMN DBAINV.TOPT_LINX_EXEC.ORIGEM IS 'Identifica o sistema origem da informação. Valores: 1=N&L, 2=MICROVIX, 3=PCI';
COMMENT ON COLUMN DBAINV.TOPT_LINX_EXEC.DESTINO IS 'Identifica o sistema destino da informação. Valores: 1=N&L, 2=MICROVIX, 3=PCI';
COMMENT ON COLUMN DBAINV.TOPT_LINX_EXEC.CADASTRO IS 'Identifica o cadastro que criou a execução. Ex.: Barras, Cest, NCM, Produtos, Pessoas, Linha, Setor, Marca';
...
COMMENT ON COLUMN DBAINV.TOPT_LINX_TRIB.ID IS 'ID da tributação no sistema LINX';
COMMENT ON COLUMN DBAINV.TOPT_LINX_TRIB.COD_GR_FISCAL IS 'Código do grupo fiscal dos itens';
COMMENT ON COLUMN DBAINV.TOPT_LINX_TRIB.COD_NBM IS 'Código NCM dos itens';
